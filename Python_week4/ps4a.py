...
def getWordScore(word, n):
    ...
    if n==len(word):
      score_total=(score*n)+50
    else:
      score_total=(score*len(word))
...
def isValidWord(word, hand, wordList):
    ...
    if word in wordList:
        for i in word:
            if i in hand2.keys() and hand2[i]>=1:
                #print (True)
                hand2[i]-=1
                bool_hand+=['True']
            else:
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    cur_hand={key:val for key,val in sorted(hand.items())}

    while calculateHandlen(cur_hand)>0:    
        # Display the hand              
        print('Current Hand: ',end=' ')
        displayHand(cur_hand)
        ...
        # Otherwise (the input is not a single period):
            ...
            # Otherwise (the word is valid):
                ...
                print('')
                # Update the hand 
                cur_hand=updateHand(cur_hand, word)
